...
export default class CryptoUtil {
    ...
    static sha256Encode(text: string, serect: string = crypto.randomBytes(32).toString("hex"),
        encoding: BinaryToTextEncoding = "base64") {

        let msg = crypto.createHmac('sha256', serect)
            .update(text)
            .digest(encoding);
    ...
    static sha256EncodeContent(str:string, encoding: BinaryToTextEncoding = "base64") {
        ...
        let msg = crypto.createHash('sha256')
        .update(str)
        .digest(encoding);
