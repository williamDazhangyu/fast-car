...
class FastCarApplication extends Events {
   ...
   constructor() {
      ...
      this.basePath = require.main?.path || "";
      this.baseFileName = require.main?.filename || "";
   ...
   updateSysConfig(sysConfig: SYSConfig, headConfig: string) {
      ...
      fileResSuffix.forEach(s => {
         let fileContent = FileUtil.getResource(
            path.join(resPath, `${headConfig}.${s}`)
         );
         if (fileContent) {
            //对于settings做特殊处理
   ...
   startLog() {
      ...
      if (!!logconfig) {
         ...
         Object.keys(logconfig.appenders).forEach(key => {
            //加入服务
            this.instanceCollection.set(
               Format.formatFirstToUp(key),
               log4js.getLogger(key)
            );
         });
   ...
   loadClass() {
      ...
      let scanPathList: string = Reflect.getMetadata(
         FastCarMetaData.ScanPathList,
         this
      );
      ...
      let excludeList: string[] = Reflect.getMetadata(
         FastCarMetaData.ScanExcludePathList,
         this
      );
      if (excludeList) {
         ...
         if (moduleClass != null) {
            moduleClass.forEach((func, name) => {
               ...
               if (this.instanceCollection.has(name)) {
                  let repeatError = new Error(
                     `Duplicate ${name} instance objects are not allowed `
                  );
                  throw repeatError;
               }
      ...
      this.instanceCollection.forEach((instance, instanceName) => {
         ...
         moduleList.forEach((name: string) => {
            ...
            if (name === "App") {
               ...
               if (!this.instanceCollection.has(name)) {
                  //找不到依赖项
                  let injectionError = new Error(
                     `Unsatisfied dependency expressed through ${name} in ${instanceName} `
                  );
   ...
   //初始化执行
   beforeStartServer() {
      this.loadSysConfig(); //加载系统配置

      this.loadClass(); //加载类

      this.loadRequireModule(); //加载依赖注入模块
   ...
   getSetting(key: string): any {
      return (
         this.sysConfig.settings.get(key) ||
   ...
   static setLoadModuleMap(name: string) {
      ...
      let names: string[] =
         Reflect.getMetadata(loadModule, FastCarApplication) || [];
      names.push(name);
