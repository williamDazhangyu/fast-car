 ...
 */
class PgsqlMapper<T extends Object> extends BaseMapper<T> {
	@Autowired
	...
	async update({ row, where, limit, forceIndex, orders }: SqlUpdate & { forceIndex?: string[]; orders?: OrderType }, @DSIndex ds?: string, @SqlSession sessionId?: string): Promise<boolean> {
		...
		let sql = `UPDATE ${this.tableName} ${forceIndexStr} SET ${rowStr.sql} `;

		if (limitStr.str || ordersStr.length > 0) {
			sql += `WHERE ctid = any(array(select ctid from ${this.tableName} ${whereC.sql} ${ordersStr} ${limitStr.str})) `;
		} else {
			sql += `${whereC.sql}`;
		}

		let res = await this.dsm.exec({ sql, args: [...rowStr.args, ...whereC.args, ...limitStr.args], ds, sessionId });
	...
	async exist(where: SqlWhere, @DSIndex ds?: string, @SqlSession sessionId?: string): Promise<boolean> {
		...
		let sql = `SELECT EXISTS (SELECT 1 FROM ${this.tableName} ${whereC.sql} LIMIT 1)`;
		let res = await this.dsm.exec({ sql, args, ds, sessionId });
	...
	async count(where: SqlWhere, @DSIndex ds?: string, @SqlSession sessionId?: string): Promise<number> {
		...
		return parseInt(rows.rows[0].num || 0);
	}

	/***
	 * @version 1.0 按照条件删除记录
	 */
	async delete(conditions: SqlDelete & { forceIndex?: string[]; orders?: OrderType }, @DSIndex ds?: string, @SqlSession sessionId?: string): Promise<boolean> {
		let forceIndexStr = this.analysisForceIndex(conditions?.forceIndex);
		let whereC = this.analysisWhere(conditions.where);
		let limitStr = this.analysisLimit({
			limit: conditions.limit,
		...
		let ordersStr = this.analysisOrders(conditions.orders);

		let sql = `DELETE FROM ${this.tableName} ${forceIndexStr} `;
		...
		let res = await this.dsm.exec({ sql, args: [...whereC.args, ...limitStr.args], ds, sessionId });
		return !!res.rowCount;
	}
	 ...
	 */
	async query(sql: string, args: any[] = [], @DSIndex ds?: string, @SqlSession sessionId?: string): Promise<pg.QueryResult<any>> {
		let res = await this.dsm.query({ sql, args, ds, sessionId });
		return res;
	}
}

export default PgsqlMapper;
