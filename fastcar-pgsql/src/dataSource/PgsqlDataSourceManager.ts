...
import { PgSqlConfig, PgSqlConfigDefault } from "../type/SqlConfig";
...
@EnableScheduling
@BeanName("PgsqlDataSourceManager")
class PgsqlDataSourceManager implements DataSourceManager {
	@Autowired
	...
	async connExecute(conn: pg.PoolClient, sql: string, args: any[] = []) {
		//打印sql
		let finalSQL = "";
		if (this.config.printSQL) {
			finalSQL = PgsqlDataSource.format(sql, args);
			this.sysLogger.info(finalSQL);
		}

		//检查sql执行时间
		let beforeTime = Date.now();
		let sqlRes = PgsqlDataSource.replacePlaceholders(sql, args);
		let res = await conn.query(sqlRes.sql, sqlRes.args);
	...
	start(): void {
		let config: PgSqlConfig = this.app.getSetting("pgsql");
		if (config) {
			this.config = Object.assign({}, PgSqlConfigDefault, config);
	...
	createDataSource(): void {
		...
		this.config.dataSoucreConfig.forEach((item) => {
			...
			let ds = new PgsqlDataSource(tmpConfig);
	...
	createSession(): string {
		...
		let connMap = new Map<string, pg.PoolClient[]>();
	...
	getSession(sessionId: string): Map<string, pg.PoolClient[]> {
		...
		return connMap as Map<string, pg.PoolClient[]>;
	}
	...
	//执行会话语句
	async exec({ sql, args = [], ds, sessionId }: SqlExecType): Promise<pg.QueryResult<any>> {
		if (!ds) {
			ds = this.getDefaultSoucre(this.isReadBySql(sql));
		}

		if (sessionId) {
			let connMap: Map<string, pg.PoolClient[]> = Reflect.get(this, sessionId) as Map<string, pg.PoolClient[]>;
			if (connMap) {
	...
	async query({ sql, args = [], ds, sessionId }: SqlExecType): Promise<pg.QueryResult<any>> {
		return await this.exec({ sql, args, ds, sessionId });
	}

	//执行sql
	async execute({ sql, args = [], ds }: SqlExecType): Promise<pg.QueryResult<any>> {
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				this.sysLogger.error("sql error:", PgsqlDataSource.format(sql, args));
	...
	//获取一个可用的连接
	async getConnection(name: string): Promise<pg.PoolClient | null> {
...
export default PgsqlDataSourceManager;
