...
class KoaServer extends FastCarApplication {
    ...
    loadDefaultHandlerService() {
        ...
        this.setHandlerService(HANDLER_SERVICE.ExceptionHandlerService,
            new KoaExceptionHandlerService());

        this.setHandlerService(HANDLER_SERVICE.ResultHandlerService,
            new KoaResultHandlerServiceImpl());
    }
                    ...
                    if (func) {
                        ...
                        for (let method of item.methods) {
                            ...
                            let callBack = async (ctx: any, next: Function) => {
                                ...
                                //进行参数的取值
                                let body = Object.keys(ctx.query).length > 0 ?
    ...
    async startServer() {
        ...
        let { port, name } = this.sysConfig.applicaion;
        app.listen(port, () => {
